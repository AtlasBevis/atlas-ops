#!/usr/bin/env python3
"""Groups: validate spec → list once → create missing → return full set."""
...
# Apicurio Registry: GroupId pattern ^.{1,512}$; "default" is reserved.
...
class Group:
    ...
    def __post_init__(self) -> None:
        ...
        if self.group_id in RESERVED_GROUP_IDS:
            raise ValueError(f"groupId '{self.group_id}' is reserved by Apicurio Registry")
...
def load_groups() -> list[Group]:
    """Load and validate groups from groups/spec.yaml (fail before any API call)."""
    ...
    for i, entry in enumerate(rows):
        ...
        group = Group(group_id=group_id, description=description)
